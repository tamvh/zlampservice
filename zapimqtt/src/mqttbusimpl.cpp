...
MQTTClientImpl::MQTTClientImpl(const std::string& serverURI, const std::string& clientId, Persistence persistence, const std::string& persistencePath, const ConnectOptions& connectOptions):
    ...
    _logger(Poco::Logger::get("system.messagebus.MQTTClient")) {
...
std::vector<TopicQoS> MQTTClientImpl::subscribedTopics() const {
    ...
    for (std::map<std::string, int>::const_iterator it = _subscribedTopics.begin();
         it != _subscribedTopics.end();
         ++it) {
...
void MQTTClientImpl::connectImpl(const ConnectOptions& options) {
    ...
    for (std::vector<std::string>::const_iterator it = options.serverURIs.begin();
         it != options.serverURIs.end();
         ++it) {
...
int MQTTClientImpl::publishMessage(const std::string& topic, const MQTTMessage& message) {
    connect();
...
void MQTTClientImpl::subscribeMany(const std::vector<TopicQoS>& topicsAndQoS)
{
    ...
    std::vector<int> qoss;
    for (std::vector<TopicQoS>::const_iterator it = topicsAndQoS.begin();
         it != topicsAndQoS.end();
         ++it) {
...
void MQTTClientImpl::resubscribe() {
    ...
    int rc = MQTTClient_subscribeMany(_mqttClient,
                                      static_cast<int>(ctopics.size()),
                                      &ctopics[0],
                                      &qoss[0]);
    if (rc != MQTTCLIENT_SUCCESS) {
...
void MQTTClientImpl::onMessageDelivered(void* context, int token) {
    MQTTClientImpl* pThis = reinterpret_cast<MQTTClientImpl*>(context);
    MQTTMessageDeliveredEvent event;
    event.token = token;
    try {
...
int MQTTClientImpl::onMessageArrived(void* context, char* topicName, int topicLen, MQTTClient_message* message) {
    ...
    if (message->payload && message->payloadlen) {
        event.message.payload.assign(static_cast<char*>(message->payload),
                                     static_cast<std::string::size_type>(message->payloadlen));
    }
